...
void transformBorderPixels(SDL_Surface *image, int borderWidth) {
    // Vérifier que l'image existe et que le bord est valide
    ...
    // Parcourir les pixels des bords
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            if (x < borderWidth || x >= width - borderWidth || y < borderWidth || y >= height - borderWidth) {
                // Le pixel est sur le bord de l'image
                ...
                if ((r == 0 && g == 0 && b == 0) || (r == 255 && g == 0 && b == 0)) {
                    // Changer la couleur du pixel en blanc
